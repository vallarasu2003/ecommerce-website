...
function  Userlist(){
    ...
    <button  className="  bg-blue-600 text-white px-4 py-2 rounded " onClick={()=>navigate("/Product")}> Back</button>
        ...
        <div className=" absolute top-20 flex flex-wrap gap-6 px-10 py-6 bg-white text-blue-600 w-full z-[20] ">
            <div  className=" shadow p-4 rounded-lg  border border-gray-300 w-full space-y-4">                       
                <p className="text-gray-700"><span className="font-semibold">User Name</span> {}</p>
                <p className="text-gray-600"><span className="font-semibold">Role</span> {}</p>
                <button className="bg-blue-600  text-white px-4 py-2 rounded" onClick={()=>navigate("/Admin")}>Edit</button>
            </div>
        </div>
    </>
    )
